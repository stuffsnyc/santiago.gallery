...
const PRODUCTS_BY_COLLECTION_QUERY = `
  query getProductsByCollection($collectionIds: [ID!]!, $first: Int!) {
...
export async function fetchProductsByCollections(collectionIds: string[], count: number = 10): Promise<any> {
  const response = await fetch(`https://${SHOPIFY_DOMAIN}/api/2023-10/graphql.json`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({
      ...
      variables: { collectionIds, first: count },
    }),
  });

  if (!response.ok) {
    throw new Error(`Failed to fetch products by collections: ${response.statusText}`);
  }
