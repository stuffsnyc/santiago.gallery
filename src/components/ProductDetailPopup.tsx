...
export function ProductDetailPopup({ product, allProducts, isOpen, onClose }: ProductDetailPopupProps) {
  ...
  useEffect(() => {
    const loadRelatedProducts = async () => {
      ...
      try {
        // First, try to find related products from already loaded allProducts
        const mainCollections = new Set(
          product.collections.edges.map(edge => edge.node.id)
        );
        
        const localRelated = allProducts
        ...
        // If we have enough related products locally, use them
        ...
        // Otherwise, fetch more from the same collections
        const collectionIds = product.collections.edges.map(edge => edge.node.id);
        
        if (collectionIds.length > 0) {
          const response = await fetchProductsByCollections(collectionIds, 10);
          const fetchedProducts: ShopifyProduct[] = [];
          
          // Extract products from all collection responses
          response.data.nodes.forEach((collection: any) => {
            if (collection && collection.products) {
              collection.products.edges.forEach((edge: any) => {
                const fetchedProduct = edge.node;
                // Avoid duplicates and exclude the current product
                if (fetchedProduct.id !== product.id && !fetchedProducts.find(p => p.id === fetchedProduct.id)) {
          ...
          // Combine local and fetched, remove duplicates, limit to 4
          const allRelated = [...localRelated, ...fetchedProducts]
        ...
        console.debug('Failed to fetch related products:', error);
  ...
  // Mobile scroll-to-close functionality (top and bottom)
  useEffect(() => {
    let touchStartY = 0;
    let touchCurrentY = 0;
    let isAtTop = false;
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      if (!isMobile || !dialogContentRef.current) return;
      ...
      isAtTop = scrollTop <= 5; // Consider "at top" if within 5px
      isAtBottom = scrollTop >= (scrollHeight - clientHeight - 5); // Consider "at bottom" if within 5px
    };
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      // Top scroll-to-close: If user is at the top and scrolling down (pulling down gesture)
      if (isAtTop && touchDiff > 80) {
        e.preventDefault(); // Prevent default scroll behavior
        triggerHapticFeedback([10, 50, 10]); // Triple tap feedback
        onClose();
        return;
      }
      
      // Bottom scroll-to-close: If user is at the bottom and scrolling up (pulling up gesture)
      if (isAtBottom && touchDiff < -80) {
        e.preventDefault(); // Prevent default scroll behavior
        triggerHapticFeedback([10, 50, 10]); // Triple tap feedback
        onClose();
        return;
          ...
          >
            {carouselProducts.map((carouselProduct, productIndex) => (
              <div key={carouselProduct.id} className="w-full flex-shrink-0 snap-start">
                <div 
                  ref={productIndex === currentProductIndex ? (el) => {
                ...
                >
                  <div className="flex flex-col lg:grid lg:grid-cols-2 lg:min-h-full">
                    {/* Image Gallery */}
                    <div className="relative bg-background overflow-hidden lg:sticky lg:top-0 flex-shrink-0">
                      {/* No related products message */}
                      {carouselProducts.length === 1 && !isLoadingRelated && (
                        <div className="absolute bottom-2 sm:bottom-4 left-1/2 transform -translate-x-1/2 z-20">
                          <div className="bg-background/90 backdrop-blur-sm rounded-full px-3 py-2 shadow-lg border border-border">
                            <p className="text-xs text-muted-foreground">No related products found</p>
                          </div>
                        </div>
                      )}
                      
                      {/* Loading message */}
                      {isLoadingRelated && carouselProducts.length === 1 && (
                        <div className="absolute bottom-2 sm:bottom-4 left-1/2 transform -translate-x-1/2 z-20">
                          <div className="bg-background/90 backdrop-blur-sm rounded-full px-3 py-2 shadow-lg border border-border">
                            <p className="text-xs text-muted-foreground">Finding related products...</p>
                          </div>
                        </div>
                      )}

                      {/* Product Navigation Dots - Bottom of Image - only show if there are related products or loading */}
                      {(carouselProducts.length > 1 || isLoadingRelated) && (
                        <div className="absolute bottom-2 sm:bottom-4 left-1/2 transform -translate-x-1/2 z-20">
                              ...
                              <div className="flex items-center space-x-[0.8vw] overflow-x-auto scrollbar-hide">
                                {carouselProducts.map((carouselProductItem, index) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <GalaxyImageWrapper 
                                        className="w-[2vw] h-[3vw] sm:w-[2.4vw] sm:h-[3.6vw] min-w-[32px] min-h-[48px] max-w-[48px] max-h-[72px]"
                                      >
                      ...
                      >
                        ...
                        {currentImages.length > 1 && (
                          <>
                            <button
                              onClick={prevImage}
                              className="absolute left-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-background/80 hover:bg-background transition-colors z-20"
                            >
                              <ChevronLeft className="w-4 h-4" />
                            </button>
                            <button
                              onClick={nextImage}
                              className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-background/80 hover:bg-background transition-colors z-20"
                            >
                          ...
                          <div className="space-y-4">
                            ...
                            {(() => {
                              ...
                              return Object.entries(optionGroups).map(([optionName, values]) => (
                                ...
                                {isLoadingRelated && relatedProducts.length < 3 && (
                                  Array.from({ length: 3 - relatedProducts.length }).map((_, index) => (
                                    <div
                                      key={`loading-${index}`}
                                      className="relative rounded-lg overflow-hidden border-2 border-transparent"
                                    >
                                      <div className="w-[2vw] h-[3vw] sm:w-[2.4vw] sm:h-[3.6vw] min-w-[32px] min-h-[48px] max-w-[48px] max-h-[72px] bg-muted animate-pulse flex items-center justify-center">
